...
function sameDayOrder(post: CollectionEntry<'posts'>) {
  if (post.id.includes('-blog-dev-')) return 0;
  if (post.id.includes('-attendance-log-')) return 1;
  return 2;
}
...
export async function getPosts() {
  return (await getCollection('posts', ({ data }) => !data.draft)).sort((a, b) => {
    ...
    const seriesDifference = sameDayOrder(a) - sameDayOrder(b);
    return seriesDifference || a.id.localeCompare(b.id, 'ko', { numeric: true });
  });
}
